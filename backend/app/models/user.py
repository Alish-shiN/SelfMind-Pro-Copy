...
class User(TimestampMixin, Base):
    __tablename__ = "users"
    __table_args__ = (
        CheckConstraint("role in ('user', 'moderator', 'admin')", name="ck_users_role_allowed"),
    )
    ...
    role: Mapped[str] = mapped_column(String(30), default=ROLE_USER, nullable=False, index=True)
