...
import { AITone, getUserPreferences, updateUserPreferences, UserPreferences } from "../api/user";
...
type Props = NativeStackScreenProps<RootStackParamList, "ProfilePersonalization">;
...
export function ProfilePersonalizationScreen({ navigation }: Props) {
  ...
  const [trustedPhoneError, setTrustedPhoneError] = useState<string | null>(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const [prefs, phone] = await Promise.all([getUserPreferences(), getTrustedPersonPhone()]);
        if (!cancelled) {
          setDraft(prefs);
        ...
        Alert.alert(t("preferencesError"), e instanceof ApiError ? e.message : t("couldNotLoadPreferences"));
      } finally {
  ...
  const save = async () => {
    ...
    try {
      ...
      Alert.alert(t("preferencesError"), e instanceof ApiError ? e.message : t("couldNotSavePreferences"));
    } finally {
      setSaving(false);
    }
  };
        ...
        <ScrollView contentContainerStyle={styles.body} keyboardShouldPersistTaps="handled">
          ...
          <View style={styles.chipWrap}>
            {supportedLanguages.map((item) => (
              <Pressable key={item.key} style={[styles.chip, language === item.key && styles.chipOn]} onPress={() => void setLanguage(item.key)}>
                <Text style={[styles.chipText, language === item.key && styles.chipTextOn]}>{item.label}</Text>
              </Pressable>
            ))}
          </View>
          ...
          <View style={styles.chipWrap}>
            {TONE_OPTIONS.map((tone) => (
              <Pressable key={tone} style={[styles.chip, draft.ai_tone === tone && styles.chipOn]} onPress={() => setDraft({ ...draft, ai_tone: tone })}>
                <Text style={[styles.chipText, draft.ai_tone === tone && styles.chipTextOn]}>{optionLabel(tone, t)}</Text>
              </Pressable>
            ))}
          </View>
          ...
          <View style={styles.chipWrap}>
            {GOAL_OPTIONS.map((goal) => {
              ...
              return (
                <Pressable key={goal} style={[styles.chip, active && styles.chipOn]} onPress={() => toggleGoal(goal)}>
                  <Text style={[styles.chipText, active && styles.chipTextOn]}>{optionLabel(goal, t)}</Text>
                </Pressable>
              );
            })}
          </View>
          ...
          {trustedPhoneError ? <Text style={styles.errorText}>{trustedPhoneError}</Text> : null}
        </ScrollView>
      ) : null}
    </SafeAreaView>
  );
...
const styles = StyleSheet.create({
  ...
  description: { color: colors.textMuted, fontWeight: "700", lineHeight: 20, marginBottom: 4 },
  sectionLabel: { fontSize: 12, fontWeight: "900", color: colors.textMuted, letterSpacing: 0.6, marginTop: 10, textTransform: "uppercase" },
  chipWrap: { flexDirection: "row", flexWrap: "wrap", gap: 8 },
  chip: { backgroundColor: colors.white, borderWidth: 1, borderColor: "#E8ECF4", borderRadius: 999, paddingHorizontal: 12, paddingVertical: 8 },
  ...
  input: { backgroundColor: colors.white, borderWidth: 1, borderColor: "#E8ECF4", borderRadius: 16, paddingHorizontal: 14, paddingVertical: 12, color: colors.text, fontWeight: "700" },
  hint: { color: colors.textMuted, fontSize: 12, fontWeight: "600", lineHeight: 17 },
