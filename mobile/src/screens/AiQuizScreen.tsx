...
export function AiQuizScreen({ navigation }: Props) {
  ...
  const selectedQuiz =
    quizTypes.find((item) => item.key === selectedQuizType) ?? quizTypes[0];
  ...
  const startQuiz = useCallback(
    async (quizType?: string) => {
      ...
      try {
        ...
        await handleAuthError(e, t("couldNotStartQuiz"));
      } finally {
        setGenerating(false);
      }
    },
  ...
  const submit = useCallback(async () => {
    ...
    try {
      ...
      await handleAuthError(e, t("couldNotSubmitQuiz"));
    } finally {
      setSubmitting(false);
    }
  ...
  const viewResult = useCallback(
    async (resultId: number) => {
      ...
      try {
        ...
        await handleAuthError(e, t("couldNotLoadQuizResult"));
      } finally {
  ...
  const createSuggestedGoal = useCallback(async () => {
    ...
    try {
      ...
      Alert.alert(t("goalCreated"), t("goalCreatedMessage"));
    } catch (e) {
      Alert.alert(
        t("couldNotCreateGoal"),
  ...
  return (
    <SafeAreaView style={styles.safe} edges={["top", "bottom"]}>
      <View style={styles.topRow}>
        <Pressable
          onPress={() =>
            mode === "landing" ? navigation.goBack() : goToLanding()
          }
          hitSlop={12}
        >
                  ...
                  >
                    ...
                    <View style={styles.typeFooter}>
                      <Text style={styles.metaText}>
                        {item.estimated_minutes} {t("minutesShort")}
                      </Text>
                      {item.latest_score != null ? (
                        <Text style={styles.metaText}>
                          {t("lastScore")} {Math.round(item.latest_score)}
                        </Text>
                      ) : null}
                      <Pressable
                        style={styles.smallBtn}
                        onPress={() => startQuiz(item.key)}
                        disabled={generating}
                      >
              ...
              {history.length ? (
                history.map((item) => (
                  ...
                  >
                    ...
                    <Text style={styles.metaText}>
                      {new Date(item.completed_at).toLocaleDateString(locale)} •{" "}
                      {titleCase(item.severity_level)} •{" "}
                      {titleCase(item.trend_direction)}
                    </Text>
        ...
        {mode === "taking" && session ? (
          <View style={styles.quizWrap}>
            ...
            <View style={styles.footer}>
              ...
              >
                <Text style={styles.submitText}>
                  {submitting ? t("loading") : t("submitQuiz")}
                </Text>
              </Pressable>
            </View>
          </View>
            ...
            <View style={styles.stack}>
              <View style={styles.resultCard}>
                ...
                <Text style={styles.resultSeverity}>
                  {titleCase(result.severity_level)}
                </Text>
                ...
                <Text style={styles.resultText}>
                  ...
                  {result.previous_score == null
                    ? "—"
                    : Math.round(result.previous_score)}{" "}
                  • {t("current")}: {Math.round(result.overall_score)} •{" "}
                  {t("difference")}:{" "}
                ...
                <Text style={styles.resultText}>
                  {result.trend_explanation || t("futureTrend")}
                </Text>
              </View>
                ...
                {(result.micro_practices || []).map((practice) => (
                  <View key={practice.title} style={styles.practiceCard}>
                    <Text style={styles.practiceTitle}>
                      {practice.title} • {practice.estimated_time}
                    </Text>
                    <Text style={styles.resultText}>
                    ...
                    {practice.action ? (
                      <Text style={styles.metaText}>
                        {t("linkedAction")}: {practice.action}
                      </Text>
                    ) : null}
                  </View>
                ))}
              ...
              {result.action_plan ? (
                <View style={styles.resultCard}>
                  ...
                  <Text style={styles.resultText}>
                    {result.action_plan.reflection_prompt}
                  </Text>
                  {result.action_plan.suggested_goal ? (
                    <View style={styles.goalBox}>
                      ...
                      <Pressable
                        style={styles.smallBtn}
                        onPress={createSuggestedGoal}
                        disabled={creatingGoal}
                      >
                        <Text style={styles.smallBtnText}>
                          {creatingGoal ? t("loading") : t("createThisGoal")}
                        </Text>
                      </Pressable>
                    </View>
                  ) : null}
                  <Text style={styles.resultText}>
                    {result.action_plan.supportive_message}
                  </Text>
                </View>
              ) : null}
              ...
              <View style={styles.footer}>
                <Pressable
                  style={styles.submitBtn}
                  onPress={() =>
                    startQuiz(result.quiz_type || selectedQuizType)
                  }
                  disabled={generating}
                >
                  <Text style={styles.submitText}>
                    {generating ? t("loading") : t("retakeQuiz")}
                  </Text>
                </Pressable>
